...
#### When u hit - npx prisma init

- that will generate 2 files schema.prisma and prisma.config
- in schema.prisma u got
...
- add that modal in that schema.prisma
...
const todos = await prisma.todo.findMany({
  where: {
    OR: [{ completed: true }, { title: { contains: 'urgent' } }],
  },
});
